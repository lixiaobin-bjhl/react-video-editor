...
export class AVCanvas {
   ...
   */
    constructor(
      ...
      = 'width: 100%; height: 100%; position: relative; overflow: hidden;'
        ...
        const { rectCtrlsGetter, destroy: ctrlGetterDestroy } = createCtrlsGetter(this.#cvsEl,)
        this.#clears.push(
            ctrlGetterDestroy,
    ...
    play(opts: { start: number; end?: number; playbackRate?: number }) {
      ...
      ?? (spriteTimes.length > 0 ? Math.max(...spriteTimes) : Infinity)
        ...
        if (opts.start >= end || opts.start < 0) {
            throw Error(`Invalid time parameter, ${JSON.stringify({ start: opts.start, end })}`,)
        }
   ...
   */
    addSprite: SpriteManager['addSprite'] = async (vs) => {
        ...
        if (clip instanceof MediaStreamClip && clip.audioTrack != null) {
            const audioNode = this.#audioCtx.createMediaStreamSource(new MediaStream([clip.audioTrack]),)
   ...
   */
    captureStream(): MediaStream {
        ...
        const ms = new MediaStream(this.#cvsEl
            .captureStream()
            .getTracks()
