import { ItemType } from '@designcombo/core'
...
export default function ControlList() {
    const {activeId, controlType, setControlType } = useEditorStore()
    useEffect(() => {
        const spr = getActiveSprite()
        if (spr) {
            setControlType(spr.controlType)
        }
        else {
            setControlType(null)
        }
    }, [activeId, controlType])

    return <>{controlType && <ControlMenu controlType={controlType} />}</>
}

function ControlMenu({ controlType }: { controlType: ItemType }) {
    const { setShowToolboxItem, setDisplayToolbox, displayToolbox, showToolboxItem, setActiveToolboxItem, activeToolboxItem }
    = useLayoutStore()
    ...
    const openToolboxItem = useCallback(
        ...
        [activeToolboxItem, showToolboxItem, displayToolbox],
    )
    return (
        <div
            style={{ zIndex: 201 }}
